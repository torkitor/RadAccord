...
**Boundary relevance.** The current MIRP adapter examines exported in-memory image/ROI evidence after native extraction. It does not inspect the subsequent NIfTI writer and therefore does not establish coverage of this incident. A separate read-back checkpoint against an independently retained affine could assess a resulting orientation discrepancy within the supported NIfTI domain.
