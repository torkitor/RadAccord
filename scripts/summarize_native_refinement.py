"""Summarize rc2 native pairs with all six checking-source hashes verified.
...
def summarize(root, run_directories, *, plan_path=None, freeze_path=None):
    ...
    freeze_path = Path(freeze_path) if freeze_path else root/'protocol/native_refinement_freeze.json'
    freeze = _read(freeze_path)
    if freeze.get('phase') != 'native-refinement-2' or freeze.get('candidate') != '1.2.0rc2':
        raise ValueError('This analysis compatibility correction supports native refinement rc2 only')
    ...
    for directory in sorted(map(Path, run_directories), key=lambda p: p.name):
        ...
        for record in records:
            ...
            expected_sources = {name: freeze['files'][relative] for name, relative in {
                ...
                'numerics': 'radaccord/numerics.py',
...
def main():
    ...
    analysis = _read(args.root/'protocol/native_refinement_analysis.json')
    selected_freeze = args.freeze or args.root/'protocol/native_refinement_freeze.json'
    if analysis.get('native_freeze_sha256') != digest(selected_freeze):
        raise ValueError('Analysis compatibility correction is bound to a different native freeze')
    for relative, expected in analysis['files'].items():
        if digest(_safe_file(args.root, relative)) != expected:
            raise ValueError('Analysis compatibility payload hash mismatch: '+relative)
    if analysis['files'].get('scripts/summarize_native_refinement.py') != digest(__file__):
        raise ValueError('This summarizer differs from the frozen compatibility correction')
