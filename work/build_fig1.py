...
text(.075, .947, 'Image pair and declared map', size=9)
text(.535, .947, 'b', size=11, weight='bold')
text(.570, .947, 'Complementary checks', size=9)
...
text(.07, .353, 'I′(j) = aI(Bj) + b', size=10)
...
cards = [
    (.704, '1', 'Physical correspondence',
     ['Positions · decoded values · ROI',
      'Full-grid match to the source',
      'Independent index evaluation']),
    (.477, '2', 'Conditional feature laws',
     ['Equivalent input: same value',
      'Known scale / intensity response',
      'Evaluate only valid relations']),
    (.250, '3', 'Absolute numerical anchors',
     ['Calculated directly from input',
      'Mean · variance · energy · volume',
      'Check for consistent output bias']),
]
...
text(.075, .174, 'Per-obligation decisions', size=9)

states = [(.05, 'Satisfied', 'within tolerance', BLUE, PALE),
          (.358, 'Violated', 'required check failed', RED, '#FDF7F6'),
          (.666, 'Not applicable', 'outside this relation', GREY, 'white')]
for x, state, note, color, fill in states:
    box(x, .043, .284, .091, edge=color, face=fill, linewidth=.65)
    text(x + .142, .101, state, size=9, weight='bold', color=RED_TEXT if state == 'Violated' else color, ha='center')
    text(x + .142, .068, note, size=8, color=INK, ha='center')
